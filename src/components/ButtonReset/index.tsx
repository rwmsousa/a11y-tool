  ...
  const reset = () => {
    ...
    if (elements) {
      for (let i = 0; i < elements.length; i += 1) {
        ...
        if (tag.getAttribute('class') === 'a11yIcon') tag.style.border = '1px solid #000000';
    ...
    // Libras
    const closeWidget: HTMLElement = document.getElementsByClassName('vpw-header-btn-close')[ 0 ] as HTMLElement;
    if (closeWidget) closeWidget.click();

    Array.from(document.getElementsByClassName('iconToolbar')).forEach((item) => {
      !item.parentElement?.classList.contains('floatingIcon') && item.setAttribute('fill', '#000000');
    });
